...
# from nets.base_UNet import UNet
...
from utils.utils import (download_weights, seed_everything, show_config,
                         worker_init_fn)
from utils.utils_fit import fit_one_epoch,fit_one_epoch_new,fit_one_epoch_new2
...
if __name__ == "__main__":
    ...
    #----------------------------------------------------------------------------------------------------------------------------#
    # 原权重 model_path  = "model_data/unet_vgg_voc.pth"
    # model_path ="/tmp/pycharm_project_51/last_epoch_miou88.892.pth"
    #model_path = "E:/unet-pytorch-main/last_epoch_miou89.170.pth"
    model_path = ""
    #-----------------------------------------------------#
    #   input_shape     输入图片的大小，32的倍数
    #-----------------------------------------------------#
    # 原形状input_shape = [512, 512]
    input_shape = [256, 256]
    ...
    #------------------------------------------------------------------#
      ...
      # 如：
      # num_classes = 3
    cls_weights = np.array([1, 10, 5,3], np.float32)
    # ------------------------------------------------------------------#
    ...
    # 原 model = Unet(num_classes=num_classes, pretrained=pretrained, backbone=backbone).train()
    model = A2FPN(class_num=num_classes).train()
    ...
    for fold, (train_index, val_index) in enumerate(kf.split(all_data)):
        print(f"Starting training on fold {fold + 1}/{K}")

        # 获取当前折的训练集和验证集
        train_fold = [all_data[i] for i in train_index]
        val_fold = [all_data[i] for i in val_index]
        ...
        train_dataset   = UnetDataset(train_fold, input_shape, num_classes, True, VOCdevkit_path)
        val_dataset     = UnetDataset(val_fold, input_shape, num_classes, False, VOCdevkit_path)
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            # fit_one_epoch_new(model_train, model, loss_history, eval_callback, optimizer, epoch,
            #         epoch_step, epoch_step_val, gen, gen_val, UnFreeze_Epoch, Cuda, loss_mode, cls_weights,
            #                               num_classes, fp16, scaler, save_period, save_dir, local_rank, best_miou)
            best_miou = fit_one_epoch_new2(model_train, model, loss_history, eval_callback, optimizer, epoch,
                    epoch_step, epoch_step_val, gen, gen_val, UnFreeze_Epoch, Cuda, loss_mode, cls_weights,
                                          num_classes, fp16, scaler, save_period, save_dir, local_rank, best_miou)
